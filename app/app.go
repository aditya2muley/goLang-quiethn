...
func getCacheStories(numStories int) ([]item, error) {
	...
	if time.Now().Sub(cacheExpTime) < 0 {
		return cache, nil
	}
	...
	cache = stories
	cacheExpTime = time.Now().Add(15 * time.Second)
	return cache, nil
}
...
func getTopStories(numStories int) ([]item, error) {
	...
	if err != nil {
		return nil, errors.New("Failed to load topStories")
	}
	var stories []item
	at := 0
...
func getStories(ids []int) []item {
	...
	for i := 0; i < len(ids); i++ {
		go func(i, id int) {
			...
			resultCh <- result{item: parseHNItem(hnItem), index: i}
		}(i, ids[i])
	}
